...
<template>
  <component
    :is="tag"
    :class="['highlight-content', init.class]"
    :style="init.style"
  >
    <template v-for="(item,index) of operateMessage">{{ item.message }}<span :key="index" :class="['highlight', init.selectedClass]">{{ item.word }}</span></template>
  </component>
</template>
